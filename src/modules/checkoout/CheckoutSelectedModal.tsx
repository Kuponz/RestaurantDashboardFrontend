import { Box, Button, Checkbox, Divider, FormControlLabel, FormLabel, IconButton, Paper, Stack, Switch, TextField, Typography } from "@mui/material";
...
export default function CheckoutSelectedIteamModal({orderValue,val,variableip,index,setValue,setcheckoutitemmodelopen}){
            ...
            <IconButton
                ...
                onClick={async () => {
                    if (orderValue?.item?.selected?.length > 0) {
                        ...
                        console.log({
                            val: val[index].item.selected[val[index].item.selected.length - 1],
                        });
                        let newValInd = {
                            ...val[index].item.selected[val[index].item.selected.length - 1],
                            id: uuidv4(),
                        };
  ...
  divider={<Divider orientation="vertical" flexItem />}>
                {val[index].complimentary.map((value)=>{
                 ...
                 <Stack direction="column" spacing={2}>
                   <Paper elevation={6}>
                     <Typography my={0.5}>{orderValue?.item?.itemName} - {value.id}</Typography>
                     <FormControlLabel
                  sx={{
                  ...
                  control={
                    <Checkbox
                      ...
                      onChange={(e) => {
                        ...
                        console.log(value);
                        updateComplimentary();
                        //    val[index].isComplimentary=e.target.checked;
